...
export default function StaffRegister({ auth }) {
    const { data, setData, post, processing, errors } = useForm({
        name: '',
        email: '',
        password: '',
        password_confirmation: '',
        role: 'staff', // hardcoded role
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('register.staff'));
    };

    // Add ?role=staff query param so backend knows this is staff login
    const handleGoogleLogin = () => {
        window.location.href = '/auth/google?role=staff';
    };
            ...
            <div className="min-h-screen bg-white text-black">
                ...
                <main className="flex flex-col items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
                    <div className="w-full max-w-md space-y-6">
                        <h2 className="text-center text-3xl font-extrabold">Staff Registration</h2>
