...
import Header from '@/Components/Header'; // Optional, remove if unused

export default function OwnerRegister({ auth }) {
    const { data, setData, post, processing, errors } = useForm({
        name: '',
        email: '',
        password: '',
        password_confirmation: '',
        role: 'owner',  // hardcoded role
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('register.owner'));
    };

    // UPDATED: pass role=owner in the URL
    const handleGoogleLogin = () => {
        window.location.href = '/auth/google?role=owner';
    };

    return (
        <>
            <Head title="Owner Registration" />
            <div className="min-h-screen bg-white text-black">
                <Header auth={auth} />

                <main className="flex flex-col items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
                    <div className="w-full max-w-md space-y-6">
                        <h2 className="text-center text-3xl font-extrabold">Admin Registration</h2>
